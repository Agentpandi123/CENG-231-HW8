...
{
    int n, server_socket_fd, data_socket_fd, port_no;
  ...
  /*********************************************************************/
  /* Add support for detecting command-line arguments here...          */
  /********************************************************************/
    if (argc < 2)
    {
    ...
    if (strcmp(argv[1], "-p") == 0)
    {
        ...
        if (port_no <= 2000)
        {
            FatalError(argv[0], "Invalid port number, should be greater than 2000");
        }
    }
  ...
  /********************************************************************/
    ...
    {
        FatalError(argv[0], "Socket failed\n");
    }
    ...
    {
        FatalError(argv[0], "binding failed\n");
    }

    printf("%s: listening on port %d...\n", argv[0], port_no);
    if (listen(server_socket_fd, 5) < 0)
    {
        FatalError(argv[0], "listening failed\n");
    }
    ...
    printf("%s: Connection to client established\n", argv[0]);
  ...
  while(strcmp(buffer, "CLOSE") != 0)
  {
    ...
    n = read(data_socket_fd, buffer, BUF_SIZE - 1);
    if (n < 0)
    {
    ...
    if (strcmp(buffer, "AUTOTEST") != 0)
    {
        if(AutoTest() != 0)
        {
            FatalError(argv[0], "Autotest Failed");
        }
    ...
    else if (strcmp(buffer, "PRINT") == 0)
    {
        printf("%s: Sending response: OK\n", argv[0]);
        memset(buffer, 0, sizeof(buffer));
        n = write(data_socket_fd, "OK\n----------------------------\nAll DIP Operations:\n - Age the image\n - Change the color image to black and white\n - Make a negative of the image\n - Flip the image Vertically\n - Mirror the image horizontally\n - Sharpen the image\n - Exchange the red and blue color channels\n----------------------------", 314);
    }
    else if (strcmp(buffer, "CLOSE") == 0)
    {
        printf("%s: Sending response: UNRECOGNIZED COMMAND\n", argv[0]);
        memset(buffer, 0, sizeof(buffer));
        n = write(data_socket_fd, "UNRECOGNIZED COMMAND", 20);
        FatalError(argv[0], "Unrecognized command");
    }
    if(n < 0)
    {
...
void PrintUsage()
{
    printf("Usage: PhotoLab_server -p <port_number>\n");
    printf("options:\n");
